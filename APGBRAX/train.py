...
from . import networks as apg_networks
...
def train(
  ...
  def env_step(
    ...
    jax.debug.print("New state obs: {}", nstate.obs)
    print("New state obs: {}", nstate.obs)
  ...
  def minibatch_step(carry, epoch_step_index: int):
    ...
    jax.debug.print("reached the minibatch step:{}",1)
    print("reached the minibatch step:{}",1)
    key, key_grad = jax.random.split(key)
    grad, (obs, state_h) = loss_grad(
        policy_params, normalizer_params, state, key_grad
    )
    print('after the minibatch_step')
    
    jax.debug.print("after the minibatch step:{}",1)
  ...
  def training_epoch(
    ...
    jax.debug.print("jax trainng epoch step:{}",1)
    (
        optimizer_state,
        normalizer_params,
        policy_params,
  ...
  def training_epoch_with_timing(
    ...
    #print('trainstate', training_state)
  ...
  for it in range(num_evals_after_init):
    ...
    print('after training_epoch_with')
    if process_id == 0:
      # Run evals.
      print('process')
