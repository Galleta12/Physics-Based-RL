...
class HumanoidPPOEval(PipelineEnv):
    def __init__(
      self,
      **kwargs,
    ):
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        jax.debug.print("new idx: {}",state.info['steps'])
        current_state_ref = self.set_ref_state_pipeline(jp.array(state.info['steps'], int),state.pipeline_state)
    ...
    def _get_obs(self, data: base.State, state_info: Dict[str, Any]) -> jax.Array:
        ...
        #phi = (jp.array(state_info['steps'],int)%self.cycle_len) / self.cycle_len
